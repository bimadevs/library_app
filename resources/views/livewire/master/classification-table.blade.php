        ...
        <table class="w-full text-left text-sm text-slate-600">
            <thead class="bg-slate-50/80 text-xs uppercase font-bold text-slate-500 tracking-wider">
                <tr>
                    ...
                    <th wire:click="sortBy('ddc_code')" class="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors group border-b border-slate-100">
                        <div class="flex items-center gap-2">
                            Kode DDC
                            <span class="text-slate-400 group-hover:text-indigo-600 transition-colors">
                                @if($sortField === 'ddc_code')
                    ...
                    <th class="px-6 py-4 border-b border-slate-100">Sub Klasifikasi</th>
                    <th class="px-6 py-4 border-b border-slate-100">Jumlah Buku</th>
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($classifications as $index => $classification)
                    <tr class="hover:bg-indigo-50/30 transition-colors duration-150">
                        <td class="px-6 py-4 text-center text-slate-400 font-mono text-xs">{{ $classifications->firstItem() + $index }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800 border border-slate-200 font-mono">
                                {{ $classification->ddc_code }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="font-semibold text-slate-800">{{ $classification->name }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-slate-600 text-sm">{{ $classification->sub_classifications_count }} sub</span>
                        </td>
                        <td class="px-6 py-4">
                            ...
                            <div class="flex items-center justify-center gap-2">
                                <button type="button" 
                                        ...
                                        @click="$dispatch('open-modal-edit', { 
                                            url: '{{ route('master.classifications.update', $classification) }}', 
                                            name: '{{ addslashes($classification->name) }}',
                                            code: '{{ $classification->ddc_code }}'
                                        })"
                                ...
                                <form action="{{ route('master.classifications.destroy', $classification) }}" 
                                      method="POST" 
                        ...
                        <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-slate-400 mt-1">
                                    @if($search)
                                        Tidak ada klasifikasi yang cocok dengan "{{ $search }}"
                                    @else
                                        Belum ada klasifikasi yang ditambahkan
                                    @endif
                                </p>
                            </div>
                        </td>
