    ...
    <div class="overflow-x-auto rounded-2xl border border-slate-200 shadow-sm bg-white">
        <table class="w-full text-left text-sm text-slate-600">
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($bookSources as $index => $bookSource)
                    <tr class="hover:bg-indigo-50/30 transition-colors duration-150">
                        <td class="px-6 py-4 text-center text-slate-400 font-mono text-xs">{{ $bookSources->firstItem() + $index }}</td>
                        <td class="px-6 py-4">
                            <span class="font-semibold text-slate-800">{{ $bookSource->name }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-slate-600">{{ $bookSource->description ?? '-' }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-100">
                                {{ $bookSource->books_count }} buku
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button type="button" 
                                        ...
                                        @click="$dispatch('open-modal-edit', { 
                                            url: '{{ route('master.book-sources.update', $bookSource) }}', 
                                            name: {{ json_encode($bookSource->name) }},
                                            description: {{ json_encode($bookSource->description ?? '') }}
                                        })"
                        ...
                        <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-slate-400 mt-1">
                                    @if($search)
                                        Tidak ada sumber buku yang cocok dengan "{{ $search }}"
                                    @else
                                        Belum ada data sumber buku yang ditambahkan
                                    @endif
                                </p>
                            </div>
                        </td>
