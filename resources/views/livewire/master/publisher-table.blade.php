            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($publishers as $index => $publisher)
                    <tr class="hover:bg-indigo-50/30 transition-colors duration-150">
                        <td class="px-6 py-4 text-center text-slate-400 font-mono text-xs">{{ $publishers->firstItem() + $index }}</td>
                        <td class="px-6 py-4">
                            <span class="font-semibold text-slate-800">{{ $publisher->name }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-100">
                                {{ $publisher->books_count }} buku
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button type="button" 
                                        ...
                                        @click="$dispatch('open-modal-edit', { 
                                            url: '{{ route('master.publishers.update', $publisher) }}', 
                                            name: '{{ addslashes($publisher->name) }}' 
                                        })"
                                ...
                                <form action="{{ route('master.publishers.destroy', $publisher) }}" 
                                      method="POST" 
                        ...
                        <td colspan="4" class="px-6 py-12 text-center text-slate-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-slate-400 mt-1">
                                    @if($search)
                                        Tidak ada penerbit yang cocok dengan "{{ $search }}"
                                    @else
                                        Belum ada penerbit yang ditambahkan
                                    @endif
                                </p>
                            </div>
                        </td>
