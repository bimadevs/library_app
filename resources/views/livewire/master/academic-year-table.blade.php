        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th>Jumlah Siswa</th>
                    <th wire:click="sortBy('is_active')" class="cursor-pointer hover:bg-slate-100">
                        <div class="flex items-center gap-1">
                            Status
            ...
            <tbody>
                @forelse($academicYears as $index => $academicYear)
                    <tr>
                        <td class="text-slate-500">{{ $academicYears->firstItem() + $index }}</td>
                        <td class="font-medium text-slate-800">{{ $academicYear->name }}</td>
                        <td>{{ $academicYear->students_count }} siswa</td>
                        <td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-8 text-slate-500">
                            ...
                            @if($search)
                                Tidak ada tahun ajaran yang cocok dengan pencarian "{{ $search }}"
                            @else
