<x-app-layout>
    <div x-data="{ 
        ...
        openCreate() {
            this.isEdit = false;
            this.formAction = '{{ route('master.classifications.store') }}';
        ...
        openEdit(event) {
            ...
            this.classificationName = event.detail.name;
            this.classificationCode = event.detail.code;
            this.open = true;
        }
    }" 
    x-init="
        @if($errors->any() && !old('_method'))
            openCreate();
            classificationName = '{{ old('name') }}';
            classificationCode = '{{ old('ddc_code') }}';
        @endif
    "
        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/60 overflow-hidden border border-slate-100">
            <div class="p-6 md:p-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h3 class="text-2xl font-bold text-slate-800 tracking-tight">Daftar Klasifikasi DDC</h3>
                        <p class="text-slate-500 mt-1">Kelola dan atur klasifikasi DDC buku perpustakaan Anda.</p>
                    </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div x-show="open"
                    ...
                    <div class="bg-white px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 z-10">
                        <div>
                            <h3 class="text-xl font-bold text-slate-800" id="modal-title" x-text="isEdit ? 'Edit Klasifikasi' : 'Tambah Klasifikasi Baru'"></h3>
                            <p class="text-sm text-slate-500 mt-1" x-text="isEdit ? 'Perbarui informasi klasifikasi yang dipilih.' : 'Tambahkan klasifikasi baru ke koleksi.'"></p>
                        </div>
                    ...
                    <div class="px-6 py-6 bg-slate-50/50">
                        <form :action="formAction" method="POST">
                            ...
                            <div class="space-y-5">
                                <div class="space-y-2">
                                    <label for="ddc_code" class="block text-sm font-semibold text-slate-700">Kode DDC <span class="text-rose-500">*</span></label>
                                    <div class="relative">
                                        <input type="text" 
                                               name="ddc_code" 
                                               id="ddc_code" 
                                               x-model="classificationCode"
                                               class="block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-3 px-4 transition-shadow hover:shadow-md focus:shadow-lg font-mono" 
                                    ...
                                    <p class="text-xs text-slate-500 pl-1">Kode DDC harus unik (3 digit).</p>
                                </div>

                                <div class="space-y-2">
                                    <label for="name" class="block text-sm font-semibold text-slate-700">Nama Klasifikasi <span class="text-rose-500">*</span></label>
                                    <div class="relative">
                                        <input type="text" 
                                               name="name" 
                                               id="name" 
                                               x-model="classificationName"
                            ...
                            <div class="mt-8 flex flex-row-reverse gap-3">
                                <button type="submit" 
                                    ...
                                    <span x-text="isEdit ? 'Simpan Perubahan' : 'Simpan Klasifikasi'"></span>
                                </button>
                                <button type="button" 
                                        @click="open = false"
