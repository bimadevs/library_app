<x-app-layout>
    <div x-data="{ 
        ...
        openCreate() {
            this.isEdit = false;
            this.formAction = '{{ route('master.classes.store') }}';
            this.className = '';
            this.open = true;
        },
        openEdit(event) {
        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/60 overflow-hidden border border-slate-100">
            <div class="p-6 md:p-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h3 class="text-2xl font-bold text-slate-800 tracking-tight">Daftar Kelas</h3>
                        <p class="text-slate-500 mt-1">Kelola dan atur data kelas perpustakaan Anda.</p>
                    </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div x-show="open"
                    ...
                    <div class="bg-white px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 z-10">
                        <div>
                            <h3 class="text-xl font-bold text-slate-800" id="modal-title" x-text="isEdit ? 'Edit Kelas' : 'Tambah Kelas Baru'"></h3>
                            <p class="text-sm text-slate-500 mt-1" x-text="isEdit ? 'Perbarui informasi kelas yang dipilih.' : 'Tambahkan kelas baru ke sistem.'"></p>
                        </div>
                    ...
                    <div class="px-6 py-6 bg-slate-50/50">
                        <form :action="formAction" method="POST">
                            ...
                            <div class="space-y-2">
                                <label for="name" class="block text-sm font-semibold text-slate-700">Nama Kelas <span class="text-rose-500">*</span></label>
                                <div class="relative">
                                    <input type="text" 
                                           name="name" 
                                           id="name" 
                                ...
                                <p class="text-xs text-slate-500 pl-1">Nama kelas harus unik.</p>
                            </div>
                            ...
                            <div class="mt-8 flex flex-row-reverse gap-3">
                                <button type="submit" 
                                    ...
                                    <span x-text="isEdit ? 'Simpan Perubahan' : 'Simpan Kelas'"></span>
                                </button>
                                <button type="button" 
                                        @click="open = false"
