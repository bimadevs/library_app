<x-app-layout>
    <div x-data="{ 
        ...
        openCreate() {
            this.isEdit = false;
            this.formAction = '{{ route('master.majors.store') }}';
            this.majorName = '';
            this.open = true;
        },
        openEdit(event) {
            ...
            this.majorName = event.detail.name;
            this.open = true;
        }
    }" 
    x-init="
        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/60 overflow-hidden border border-slate-100">
            <div class="p-6 md:p-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h3 class="text-2xl font-bold text-slate-800 tracking-tight">Daftar Jurusan</h3>
                        <p class="text-slate-500 mt-1">Kelola dan atur jurusan sekolah Anda dengan mudah.</p>
                    </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div x-show="open"
                    ...
                    <div class="bg-white px-6 py-5 border-b border-slate-100 flex items-center justify-between sticky top-0 z-10">
                        <div>
                            <h3 class="text-xl font-bold text-slate-800" id="modal-title" x-text="isEdit ? 'Edit Jurusan' : 'Tambah Jurusan Baru'"></h3>
                            <p class="text-sm text-slate-500 mt-1" x-text="isEdit ? 'Perbarui informasi jurusan yang dipilih.' : 'Tambahkan jurusan baru ke sistem.'"></p>
                        </div>
                    ...
                    <div class="px-6 py-6 bg-slate-50/50">
                        <form :action="formAction" method="POST">
                            ...
                            <div class="space-y-5">
                                <!-- Name Field -->
                                <div>
                                    <label for="name" class="block text-sm font-semibold text-slate-700 mb-2">Nama Jurusan <span class="text-rose-500">*</span></label>
                                    <div class="relative">
                                        <input type="text" 
                                               name="name" 
                                               id="name" 
                                    ...
                                    <p class="text-xs text-slate-500 mt-1 pl-1">Nama lengkap jurusan sesuai kurikulum.</p>
                                </div>
                            </div>
                            ...
                            <div class="mt-8 flex flex-row-reverse gap-3">
                                <button type="submit" 
                                    ...
                                    <span x-text="isEdit ? 'Simpan Perubahan' : 'Simpan Jurusan'"></span>
                                </button>
                                <button type="button" 
                                        @click="open = false"
