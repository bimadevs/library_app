<aside x-data="{ 
        ...
        <div class="flex items-center gap-3 transition-opacity duration-300" :class="{ 'opacity-0 invisible': collapsed }">
            <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden">
                @if(isset($librarySetting) && $librarySetting->logo_url)
                    <img src="{{ $librarySetting->logo_url }}" alt="Logo" class="w-8 h-8 object-contain">
                @else
            ...
            <div class="whitespace-nowrap overflow-hidden max-w-[140px]">
                <h1 class="font-bold text-lg truncate" title="{{ $librarySetting->school_name ?? 'Perpustakaan' }}">
                    {{ $librarySetting->school_name ?? 'Perpustakaan' }}
                </h1>
    ...
    <nav class="mt-4 px-3 overflow-y-auto h-[calc(100vh-100px)] overflow-x-hidden">
        ...
        <a href="{{ route('settings.index') }}"
           class="sidebar-link mt-2 {{ request()->routeIs('settings.*') ? 'active' : '' }}"
           :class="{ 'justify-center': collapsed }"
           wire:navigate
           title="Pengaturan Identitas">
            ...
            <span x-show="!collapsed" class="ml-3 transition-opacity duration-300">Pengaturan</span>
        </a>
